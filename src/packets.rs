...
use mysql_common::{
    io::ParseBuf, misc::raw::{
        bytes::{EofBytes, NullBytes, U8Bytes}, int::{LeU16, LeU32, LenEnc}, Const, RawBytes, RawConst, RawInt
    }, packets::{AuthPlugin, ErrPacketHeader, ProgressReport, SqlState}, proto::{MyDeserialize, MySerialize}
};
...
pub struct OldHandshakeResponse<'a> {
...
impl<'de> MyDeserialize<'de> for OldHandshakeResponse<'de> {
    const SIZE: Option<usize> = None;
    type Ctx = ();

    fn deserialize((): Self::Ctx, buf: &mut ParseBuf<'de>) -> io::Result<Self> {
        // let mut sbuf: ParseBuf<'_> = buf.parse(4 + 4 + 1 + 23)?;
        let client_flags: RawConst<LeU32, CapabilityFlags> = buf.parse(())?;
        let max_packet_size: RawInt<LeU32> = buf.parse(())?;
        if max_packet_size > RawInt::new(0xFFFFFF) {}
        let user = sbuf.parse(())?;
